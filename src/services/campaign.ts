...
export default class CampaignService {
    ...
    public getCampaignDetails = (async (campaignId: string, next: NextFunction) => {
        try {
            ...
            if (!data) return getErrorMessage(next, "campaign not found", StatusCodes.NOT_FOUND)
            return data
        } catch (error) {
    ...
    public getCampaignMetrics = (async (campaignId: string, next: NextFunction) => {
        try {
            ...
            if (!campaign) return getErrorMessage(next, "campaign not found", StatusCodes.NOT_FOUND)
